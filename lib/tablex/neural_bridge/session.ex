defmodule NeuralBridge.Session do
  @moduledoc """
  ...
  @defined_functions NeuralBridge.DefinedFunctions
  ...
  @spec new(String.t()) :: t()
  def new(id), do: %Session{id: id, rule_engine: Retex.new()}
  ...
  def apply_rules(session = %__MODULE__{rule_engine: rule_engine, rules_fired: rules_fired}) do
    rules = extract_applicable_rules(rule_engine, rules_fired)
  ...
  defp extract_applicable_rules(%_{agenda: agenda}, rules_fired) do
    agenda -- rules_fired
  end
